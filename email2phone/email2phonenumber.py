...
def get_masked_email_amazon(phone_numbers, victim_email, verbose):
    ...
    for phone_number in phone_numbers:
        ...
        if response.status_code >= 500:
            ...
            if verbose:
                print("%sPhone %s not registered %s",
                            YELLOW, phone_number, ENDC)
            continue
            ...
            if verbose:
                print("%sCaptcha caught us trying number: %s %s",
                            YELLOW, phone_number, ENDC)
            continue
            ...
            if \
                ...
                print("%sPossible phone number for %s is: %s %s",
                            GREEN, victim_email, phone_number, ENDC)
                found_possible_number = True
            else:
                if verbose:
                    print("%sNo match for email: %s and number: %s %s",
            ...
            print("%sUnknown error: %s", RED, ENDC)
            if verbose:
                print("%s%s %s", RED, response.text, ENDC)
            exit("Unknown error!")
...
def scrape_last_pass(email):
    ...
    if regex_output and regex_output.group(3):
        ...
        print("%sLastpass reports that the last 2 digits are: %s %s",
        ...
        print("%sLastpass did not report any digits %s", YELLOW, ENDC)
...
def scrape_ebay(email):
    ...
    if regex_output and regex_output.group(1):
        ...
        print("%sEbay did not report any digits %s", YELLOW, ENDC)
        return
    ...
    if regex_output:
        if regex_output.group(1):
            ...
            print("%sEbay reports that the first digit is: %s %s",
                        GREEN, first_digit, ENDC)
        if regex_output.group(2):
            last_two_digits = regex_output.group(2)
            print("%sEbay reports that the last 2 digits are: %s %s",
                        GREEN, last_two_digits, ENDC)
    else:
        print("%sEbay did not report any digits %s", YELLOW, ENDC)
...
def scrape_paypal(email):
    ...
    if regex_output and regex_output.group(1):
        ...
        print("%sPaypal did not report any digits %s", YELLOW, ENDC)
        return
    ...
    if regex_output and regex_output.group(3):
        ...
        print("%sPaypal reports that the last %s digits are: %s %s",
...
def set_proxy_list():
    ...
    if not proxy_file.mode == 'r':
        proxy_file.close()
        print("%sCould not read file %s %s", RED, args.proxies, ENDC)
...
try:
    ...
    print("Please, add an option to execute")
    parser.print_help()
    sys.exit()
...
if args.action == "scrape":
    ...
    if args.file and not os.path.isfile(args.file):
        print("%sYou need to pass a valid file path %s", RED, ENDC)
        exit()

    print("Looking for the phone number associated to %s ...", args.email)
    if args.mask:
        ...
        if not f.mode == 'r':
            f.close()
            print("%sCould not read file %s %s", RED, args.file, ENDC)
            exit()
        file_content = f.read()
    ...
    print("%sAction not recognized", RED, ENDC)
    exit()
